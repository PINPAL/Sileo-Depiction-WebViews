...
function getRepo(repoURL) {
  ...
  try {
    ...
    // Fetch a plaintext file from the repo
    fetch(repoURL + "Release").then(function (response) {
      response.text().then(function (text) {
        repoReleaseTextString = text;
        repoTitle = repoReleaseTextString
          .match(/Origin:.*/)[0]
          .replace(/Origin:\s/g, "");
        // Set Repo Title
    ...
    reloadingRepoText.innerText = "Failed to load Repo Title: " + error;
    throw "Failed to load Repo Title" + error;
  }
  ...
  try {
    ...
    // Fetch a plaintext file from the repo
    fetch(repoURL + "Packages").then(function (response) {
      response.text().then(function (text) {
        repoPackagesFile = text;
        packages = decodePackagesFile(repoPackagesFile);
        renderPackagesFile(packages);
      });
    });
  } catch (error) {
    reloadingRepoText.innerText = "Failed to load Packages: " + error;
    throw "Failed to load Packages" + error;
  }
  // Update Loading Progress Text
  reloadingRepoText.innerText = "Fetching Sileo Featured";
  // Fetch Sileo Featured JSON from Repo
  try {
    let sileoFeaturedJSON = null;
    // Fetch a plaintext file from the repo
    fetch(repoURL + "sileo-featured.json").then(function (response) {
      response.text().then(function (text) {
        sileoFeaturedJSON = text;
        sileoFeaturedJSON = JSON.parse(sileoFeaturedJSON);
        renderSileoFeatured(sileoFeaturedJSON, packages);
      });
    });
  } catch (error) {
    reloadingRepoText.innerText = "Failed to load Sileo Featured: " + error;
    throw "Failed to load Sileo Featured" + error;
  }
  // Hide the reloading indicator
  document.getElementById("reloadingRepoWrapper").style.visibility = "none";
}
...
if (repoURL) {
  ...
  document.getElementById("reloadingRepoWrapper").style.visibility = "visible";
        ...
        if (packages[j].Section == categories[i].name) {
          ...
          if (packages[j].hasOwnProperty("SileoDepiction")) {
            ...
            if (packages[j].hasOwnProperty("Author")) {
              ...
              tableCellLink.href +=
                "&dev=" + packages[j].Author.replace(/\<.*\>/g, "");
            }
            // Add Icon to HREF
...
function renderSileoFeatured(sileoFeaturedJSON, packages) {
  if (sileoFeaturedJSON != null) {
    ...
    with (bannerImage) {
      ...
      style.height =
        sileoFeaturedJSON.itemSize.replace(/\{.*\,|\}|\s/g, "") + "px";
    }
    ...
    for (i = 0; i < sileoFeaturedJSON.banners.length; i++) {
      ...
      bannerImageClone.style.backgroundImage =
        "url('" + sileoFeaturedJSON.banners[i].url + "')";
      ...
      if (packages != null) {
        for (packageNum = 0; packageNum < packages.length; packageNum++) {
          if (
            packages[packageNum].Package == sileoFeaturedJSON.banners[i].package
          ) {
            ...
            if (packages[packageNum].hasOwnProperty("SileoDepiction")) {
              ...
              if (packages[packageNum].hasOwnProperty("Section")) {
                bannerImageLink.href +=
                  "&section=" + packages[packageNum].Section;
              }
              // Add Developer to HREF
              if (packages[packageNum].hasOwnProperty("Author")) {
                //Replace to remove emails in triangular brackets
                bannerImageLink.href +=
                  "&dev=" + packages[packageNum].Author.replace(/\<.*\>/g, "");
              }
              // Add Icon to HREF
...
function openCategory(element) {
  ...
  document
    .getElementsByClassName("leftNavButton")[0]
    .setAttribute("onclick", "back(" + categoryID + ")");
  document.getElementById("categorySubView" + categoryID).style.display =
    "inline-block";
  document.getElementById("repositoryName").innerText =
    element.getElementsByClassName("left")[0].innerText;
...
function back(categoryID) {
  document.getElementById("categorySubView" + categoryID).style.display =
    "none";
  document.getElementById("repositoryName").innerText =
    document.getElementsByClassName("leftNavButton")[0].id;
